...
from models import User, OAuth, tagsTweets, Tweet
...
def check():
    ...
    load = db.session.query(OAuth).filter(OAuth.user_id == current_user.id).first().token
    twt_user = load['user_id']
    FoO_user = current_user.id
    token = load['oauth_token']
    token_secret = load['oauth_token_secret']
    return {'token': token, 'token_secret': token_secret, 'twitter_user': twt_user, 'FeedsofOthers_user': FoO_user}, 200
